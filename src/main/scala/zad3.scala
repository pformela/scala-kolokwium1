...
object zad3 {
    ...
    def maxAvgTemps(data: List[String]): Set[(Int, Double)] = { // Set[(Int, Double)]
        data.map(n => n.split(" ").toList.tail).transpose.zipWithIndex.foldLeft(Set[(Int, Double)]())((prev, next) => {
            prev + ((next._2 + 1, next._1.map(n => n.toDouble).max))
        })
    }
...
//        println(data.head.split(" ").toList)
//        println(maxAvgTemps(data))
        val newData = data.map(n => n.split(" ").toList.tail)

        println(maxAvgTemps(data).toSeq.sortBy(_._1).toSet)
    }
}
